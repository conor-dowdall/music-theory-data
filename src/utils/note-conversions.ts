...
import { noteLetterIntegers } from "../note-labels/note-labels.ts";

/**
 * Converts a note letter and note alteration (number of semitones) to a note integer (0-11).
 *
 * @param noteLetter The note letter {@link NoteLetter} (A-G | a-g).
...
export function noteLetterToInteger(
  ...
  const noteInteger = (noteLetterIntegers[noteLetter] + NoteAlterInteger + 12) %
    12;
 ...
 * noteLetterToMidiNoteNumber("c", 0, 0);
 *
 * // Returns 60 (MIDI note number for middle C = C4)
 * noteLetterToMidiNoteNumber("C", 0, 4);
 *
 * // Returns 61 (MIDI note number for C#4)
 * noteLetterToMidiNoteNumber("C", 1, 4);
 * ```
 */
export function noteLetterToMidiNoteNumber(
  noteLetter: NoteLetter,
  noteAlterInteger: NoteAlterInteger,
  noteOctave: number,
): number {
  return (
    noteLetterToInteger(noteLetter, noteAlterInteger) + (noteOctave + 1) * 12
  );
}
...
export function simpleNoteNameToInteger(
  ...
  const noteLetter = noteLetterMatch[0] as NoteLetter;
  if (!(noteLetter in noteLetterIntegers)) {
    console.warn(`Invalid note letter: ${noteLetter}`);
    return undefined;
  }
  ...
  return ((noteLetterIntegers[noteLetter] + noteAlterInteger + 12) %
    12) as NoteInteger;
}
